​	Doug Lea的`util.concurrent`包除了包含许多其他有用的并发构造块之外，还包含了一些主要集合类型`List`和`Map`的高性能的、线程安全的实现。Brian Goetz向您展示了用`ConcurrentHashMap`替换`Hashtable`或`synchronizedMap`，将有多少并发程序获益。
